...
class GrafanaPyroscopeDataQuery(DataQuery):
    ...
    groupBy: List[str] = Field(..., description='Allows to group the results.')
    maxNodes: Optional[int] = Field(
        None, description='Sets the maximum number of nodes in the flamegraph.'
    )


class PhlareQueryType(Enum):
    both = 'both'
    profile = 'profile'
    metrics = 'metrics'
